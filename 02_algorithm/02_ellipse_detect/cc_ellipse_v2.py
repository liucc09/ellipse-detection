...
class Path(object):
    ...
    def cal_angle(self):
        ...
        if y0==y1:
            ...
            A,B,C = 1,(x1-x0)/(y0-y1),(x1*y0-x0*y1)/(y1-y0)
            
        dis_a = np.abs(np.array(self.xs)+B*np.array(self.ys)+C)/np.sqrt(1+B*B)
        self.line_rho = len(np.where(dis_a<=1.5)[0])/l #直线度
...
def cut_inflexion(p_ori):
    ...
    for id2 in peaks:
        ...
        #if path.line_rho<0.9 or len(path)<40:
        paths.append(path)
...
#p1:Path
#p2:Path
def extend_path(p1,p2,glob):
        
    ht1 = list(p1.get_head_tail())
    ht2 = list(p2.get_head_tail())
    
    len_thr = max(min(len(p1),len(p2)),glob['MAX_DIS'])
    
    tags = ['h','t']
    
    for i in range(2):
        for j in range(2):
            e1,e2,e3,e4 = ht1[i],ht2[j],ht1[1-i],ht2[1-j]
            
            if p1.uid==37 and p2.uid==136 and i==1 and j==0:
                a=1
            
            res = judge_connect(e1,e2,e3,e4,len_thr,glob['MAX_ANGLE'])
...
#判断两直线是否相交在第二条直线上
def intersectOnline(x1,y1,x2,y2,x3,y3,x4,y4):
    d1 = (x1-x2)*(y3-y4)-(y1-y2)*(x3-x4)
    d2 = (x1-x2)*(y3-y4)-(y1-y2)*(x3-x4)
    if d1==0 or d2==0:
        return True
    
    px= ( (x1*y2-y1*x2)*(x3-x4)-(x1-x2)*(x3*y4-y3*x4) ) / ( d1 ) 
    py= ( (x1*y2-y1*x2)*(y3-y4)-(y1-y2)*(x3*y4-y3*x4) ) / ( d2 )
    
    if px>=min(x3,x4) and px<=max(x3,x4) and py>=min(y3,y4) and py<=max(y3,y4):
        return True
    else:
        return False
    
#返回夹角，空缺距离，其中夹角后面算法中没有用，只用来调试方便
def judge_connect(pt1,pt2,pt3,pt4,len_thr,angle_thr=45):
    ...
    if intersectOnline(x3,y3,x4,y4,x1,y1,x2,y2):
        return None
    ...
    d12 = a2-a1
    d12 = d12-360 if d12>180 else d12
    d12 = d12+360 if d12<-180 else d12
    ...
    da1 = abs(a12-a1)
    da1 = da1-360 if da1>180 else da1
    
    
    da2 = abs(a2-a12)
    da2 = da2-360 if da2>180 else da2
    
    
    if abs(da1)>angle_thr or abs(da2)>angle_thr:
        return None
...
def rank_neibor(p):
    ...
    for nb in p.neibors:
        ...
        if l2>best_hl and tag1=='h':
            best_h = f'{tag1}:{tag2}:{uid2}:{da}'
            best_hl = l2
        elif l2>best_tl and tag1=='t':
            best_t = f'{tag1}:{tag2}:{uid2}:{da}'
            best_tl = l2
...
#将弧线连接成椭圆弧组合
#(转角):t:8:h->(转角):t:9:h->(转角):h:10:t->(转角):t:11:h
#direction:tail,head,both
...
def cangle_length(g,paths):
    ...
    for pstr in g.split('->'):
        ...
        dangle,tag1,uid,tag2 = pstr.split(':')
        
        dangle = int(dangle[1:-1])
        cangle+=dangle
        cangle_abs+=abs(dangle)
    ...
    return abs(cangle),cangle_abs,length


#计算拟合的椭圆和原始点的贴合程度
def ellipse_dis(x,y,a,b,theta,xs,ys,dis_thr=2):
    xs = np.array(xs)-x
    ys = np.array(ys)-y
    ...
    dis = np.abs(np.sqrt(np.sum(ell**2,axis=0))-100)
    
    rho0 = len(np.where(dis<=dis_thr)[0])/len(dis) #误差小于两个像素的点所占比例
...
def fit_ellipse(grp,width,glob):
    ...
    for i in range(6):
        ...
        if Ls[i]>0 and mu>0:
            ...
            #排除畸形椭圆
            if abs(Xc)>2*width or abs(Yc)>2*width or MA<=2 or SMA<=2 or MA>2*width or SMA>2*width or MA/SMA>5:
                continue
            ...
            if e<err0:
                err0 = e
                re['rho'],re['dises'] = ellipse_dis(*params,xs,ys,glob['MAX_FIT_DIS'])
                
                perimeter = 2*np.pi*SMA+4*(MA-SMA)
                re['rho2'] = re['rho']*grp['length']/perimeter #占椭圆的比例
                re['cover_rho'] = re['rho2']*re['rho']
                #re['ma_sma'] = MA/SMA
                re['param'] = params
                re['group'] = grp 
                
    if err0>1e9 or re['rho2']<glob['MIN_COVER_RHO2'] or re['rho']<glob['MIN_COVER_RHO1']:
        return None
    else:
        return re
...
def detect_ellipses_im(im,glob):
    ...
    for p in path_list:
        ...
        if len(p.neibors)==0:
            ...
            gs =  find_groups(f'(0):h:{p.uid}:t',consume,paths)
            groups += gs

            gs =  find_groups(f'(0):t:{p.uid}:h',consume,paths)
            groups += gs
    ...
    for g in groups:
        ...
        cangle,cangle_abs,length = cangle_length(g,paths)
        if (cangle_abs-cangle<45 and cangle>glob['MIN_COVER_ANGLE'] and cangle<400 and length>glob['MIN_LEN']):
        
            groups_info.append({'paths':group2list(g,paths),
                                'group':g,
                                'cover':cangle,
                                'cover_abs':cangle_abs,
                                'length':length})
