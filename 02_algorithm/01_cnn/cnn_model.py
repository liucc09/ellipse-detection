...
class RestoreNet(nn.Module):
    def __init__(self,channel=1):
        ...
        m0 = nn.Sequential(
                    ...
                    nn.Conv2d(channel, ndf, kernel_size=5, stride=1, padding=0), #100
                    nn.BatchNorm2d(ndf),
                    nn.ReLU()
                    )
        ...
        mu2u = [nn.Sequential(
                     nn.ReflectionPad2d(2),
                     nn.Conv2d(2*ndf*(2**i), ndf*(2**i), kernel_size=5, stride=1, padding=0), #100
                     nn.BatchNorm2d(ndf*(2**i)),
                     nn.ReLU(),
                    )
                  for i in range(nlayer-1,-1,-1)]
    ...
    def forward(self, x):
        ...
        for i in range(self.nlayer):
            u_ = self.u_[i](u[i])
            mu_ = torch.cat((m[self.nlayer-i-1],u_),1)
            
            u.append(self.mu2u[i](mu_))
    ...
    with torch.no_grad():
        ...
        for (image,label) in test_loader:
            ...
            for i in range(len(outs)):
                ...
                prt[i].append(outs[i].detach().cpu().numpy())
            
            og.append(np.clip(image.detach().cpu().numpy(),0,1))
            gt.append(label.numpy())
        
    pr = []
    for lt in prt:
        pr.append(np.concatenate(lt,axis=0))
        
    return np.concatenate(og,axis=0),np.concatenate(gt,axis=0),pr
...
def predict3(model,im,device):
    ...
    for r in range(0,h,width):
        for c in range(0,w,width):
            ...
            x = np.zeros((width,width,3),dtype=np.float32)
    ...
    for x in xs:
        x = np.transpose(np.expand_dims(x,0),(0,3,1,2))
        x = torch.tensor(x,dtype=torch.float32).to(device)
        
        y = model.predict(x).detach().cpu().numpy()
        ys.append(y[0,0,:,:])
        
    imo = np.zeros((h,w),dtype=np.float32)
    id_ = 0
